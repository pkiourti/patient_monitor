...
```
**NOTE**: MongoDB files are not uploaded in the repository because they are large binary files. So, I only replaced the users module to use MongoDB (even though it would be similar for patients, user_roles, messages, sessions, devices, device_assignments, device_measurements, device_types, etc.) and commented out one test from patients and the users api tests since mongodb is not running in the github workflow.
